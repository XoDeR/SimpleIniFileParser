...
namespace Rio
{
	...
	{
		assert(str != nullptr); // , "String is NULL");
		return str;
	}
	...
	{
		assert(str != NULL);// , "Str must be != NULL");
	...
	{
		assert(begin != NULL);// , "Begin must be != NULL");
		assert(end != NULL);// , "End must be != NULL");
		assert(out != NULL);// , "Out must be != NULL");
	...
	inline int32_t parseInt(const char* str)
	{
		...
		assert(ok == 1);// , "Failed to parse int: %s", str);
		RIO_UNUSED(ok);
	...
	inline uint32_t parseUint(const char* str)
	{
		...
		assert(ok == 1);// , "Failed to parse uint: %s", str);
		RIO_UNUSED(ok);
	...
	inline float parseFloat(const char* str)
	{
		...
		assert(ok == 1);// , "Failed to parse float: %s", str);
		RIO_UNUSED(ok);
